...
- **"修过洞"要区分维护勤度（2026-08 两案例修正）**：
  ...
  - **★ "被审过但没审干净" = 最佳目标（2026-08-14 buddyforms 案例）**：changelog 有明确的安全修复条目（Max Boll 审的 buddyforms_nav/bf_new_submission_link 短代码洞，2025-02）+ 之后 400+ 天没更新 = 修复后无人再审 → 新漏网温床（buddyforms 的 process_edit_post 任意文章篡改就是修复后的新漏网）。判定法：changelog 安全修复越具体（报洞人名+漏洞类型）说明作者认真修过 → 但**只修了报告点、没全量自查**是常态 → 挖"修复点之外的兄弟面"
  - **竞品反推法（2026-08-14 B 方案，实战失败但规律清晰）**：小装量插件挖到洞（buddyforms 900装）→ 推断同类大装量竞品（wp-user-frontend 2万装/user-registration 5万装）有同款洞 → 但**大装量竞品全在 1-2 天内更新（活跃维护）= 防护质量高，白查**。结论：竞品反推只对"同属老代码"的竞品有效（同一维护时代），对活跃竞品无效——别对刚更新的竞品投入
...
- **★ 表单引擎金矿池首轮实战（2026-08-14，筛选器 `scripts/wp_filter_formengine.py`）**：307 候选 → 40 个 150天+未更新。优先画像=配置驱动表单引擎 × 未认证面 × 老代码。首轮审 4 个：
  ...
  - **buddyforms 已排除面（2026-08-14 深挖，agent 并行 2 面 + 主线 3 文件）**：①注册提权✗——`buddyforms_wp_insert_user` 角色只来自表单配置 `registration.new_user_role`（wp-insert-user.php:236，默认 subscriber），全函数无 POST 读取；wp_update_user 路径角色变更需 `current_user_can('administrator')`（:92）。②file_put_contents 路径穿越✗——form_slug 来自 shortcode 参数（管理员写在页面）且渲染前有 `isset($buddyforms[$form_slug])` 存在性检查（form-render.php:96）→ 非未认证可控任意字符串。③delete_post 的 nopriv 注册被注释（form-ajax.php:221 注释掉）→ 仅登录+作者归属校验。④REST/激活链 nonce 绑定对象（buddyforms_create_nonce 传 $user_id 生成，functions.php:1255）→ 换 user 参数 nonce 不匹配。⑤模板加载硬编码模板名
  - **★ buddyforms 候选洞 A：bf_post_type + status 双 POST 可控 → 未认证创建任意 post type/状态 WP 对象（2026-08-14 主线挖到，证据完整）**：`form-control.php:260-261` `$post_type = sanitize_text_field($_POST['bf_post_type'])` 无白名单；`:130` `$post_status_action = $_POST['status']` POST 可控；`:607` `wp_insert_post($bf_post)` 直接用；`:275` post_type 校验只在"编辑已有 post"(post_id!=0)时执行，新建不校验；`:430` 未登录 `$user_id=0` → post_author=0。**前置**：站点有 public_submit 公开表单（BuddyForms 前端投稿标配）→ 表单 HTML 公开输出 `wp_nonce_field('buddyforms_form_nonce')`（form-render.php:254）+ `form_slug`/`bf_post_type` 是 hidden 字段（:266-267）→ 未认证改 hidden 值即可。**影响**：未认证创建任意 post_type（可指定 publish）——绕过"提交后审核"流直接发布，或创建表单配置之外的 post type。**判定**：机制✓；靶场可达性+真实价值待验证（900装小众，金矿面=能建 admin 自定义 post type 时价值才高；仅建 post 可能落在"任意内容创建"档，需查 Wordfence 是否收）；**大插件文件规模识别法（489 文件不可怕）**：`find . -name "*.php" | sed 's|/[^/]*$||' | sort | uniq -c | sort -rn` 按目录统计——buddyforms 489 个里 373 个是第三方库(freemius SDK/guzzle/pfbc 表单渲染库)，核心业务仅 82 个，真正未认证相关约 10 个。**文件多≠工作量大，入口点少才决定工作量**（ChainQ 700+ 文件只精读 3 个）
  - **★ 表单引擎池第二轮（2026-08-14 下午，3 连审全放弃，规律确认）**：acf-gravityforms-add-on（30000装/255天）→ 12文件600行纯 ACF 字段类型注册（无 nopriv/无 AJAX/无 REST/无短代码/唯一动作是 admin_init 加载文本域）攻击面≈0；structured-content（40000装/384天）→ SEO 短代码插件，输出全转义（esc_attr/wp_kses_post/wpsc_esc_jsonld），攻击面需 contributor+ 写 post_content → 超范围；clean-and-simple-contact-form（7000装/226天）→ 联系表单标准防护模板（IsValid=nonce+sanitize+recaptcha；SendMail 邮件头注入被 sanitize_email/sanitize_text_field 堵死换行；收件人=管理员配置；无文件无 SQL）防护完整。**规律：① 筛选器关键词命中\"form/ACF\"≠表单引擎——acf-gf 是误报，粗扫 30 秒看结构（文件数/入口点）即识别；② 大装量插件普遍防护完整（被盯得紧），有洞的 = 老代码×前端提交面×配置驱动（buddyforms 唯一命中但 900 装超范围）——\"装量÷未认证面\"错配是主要矛盾：小装量有洞交不了、大装量没洞。修正版画像：表单引擎金矿池选 25k-50k 装档（全类型可交），别在 <5k 装的小表单插件上花时间**
  - **★ 受限扩展名文件写判定法（2026-08-14 用户纠正 \"C 只能上传png 没啥用把\"）**：buddyforms upload_image_from_url（nopriv+无nonce+远程URL下载+$file_id 路径穿越）但**扩展名被强制 `.png`** → 不算\"任意 PHP 文件上传\"（High Threat 定义）→ 900 装超范围。**判定文件写/上传洞先看扩展名可控性：攻击者可定扩展名（.php 可达）= High Threat；强制受限扩展名（.png/.jpg）= 降级为\"受限文件写\"，<50k 装超范围白挖**——别被\"未认证+无nonce+路径穿越\"的静态链兴奋带偏，先算扩展名账
  - **★ 表单引擎池第三批（2026-08-14 晚间，9 目标终局：大装量×老代码=空集，筛选器路线正式判死）**：ajax-login-and-registration-modal-popup（3000装/275天）→ 4 个 nopriv 认证端点（login/signup/lostpassword/password_reset）全走 `_verify_nonce()`，但该函数开头 `if (defined("WP_CACHE")) return true;` → **WP_CACHE 常量（缓存插件定义，生产站常态）下 nonce 校验静默失效**——真实防护失效但仅 CSRF（4 端点全用 WP 核心 wp_signon/get_password_reset_key/check_password_reset_key，无直接 High Threat）；gdpr-data-request-form（5000装/255天）→ nonce+human 验证，human 答案就是 hidden 字段 N000M 拼接可自算绕过 → 仅\"向任意邮箱发 GDPR 邮件\"滥用（超范围）。**战略验证（修正筛选器 25k-50k×表单×150天+未更）**：全池仅 3 目标（structured-content 已审无面 / icon-block 块插件无面 / wp-custom-admin-interface 纯 admin）→ **大装量插件=活跃维护（竞品全在 1-2 天内更新），老代码=小装量 → \"大装量×老代码\"是空集，关键词筛选器路线正式判死**。9 目标终局：0 可交（buddyforms 2 洞真实但 900 装超范围；其余 8 个防护完整或无面）。唯一目标真实存在的路 = 0day 竞速（新插件装量 0 也 in-scope）或手动翻分类找\"功能复杂×维护差\"中型插件（接受交不了现实，练手用）。buddyforms 完整存档：D:\Pentest\审计进度\BuddyForms_审计存档_20260814.md
- **六路并行挖洞思路（2026-08-14 定；A 已实战判死，F 升级为主力）**：A ChainQ复制=表单引擎筛选器 → **判死（2026-08-14 晚验证：25k-50k×表单×150天+未更全池仅 3 目标且全无面，大装量×老代码=空集）** → B 远古文件操作(1k-5k装×文件操作×3-5年未更) 尚未系统打 → C 补丁分析池(已退役) → D SQLi/XSS面(10k-50k大插件, 机械层实测干净, 逻辑层才是高发) → E 认证面(密码重置/登录/会员) → **F 0day竞速(新插件装量0也in-scope) = 唯一目标真实存在且装量不卡人的路，升级为主力**。详表见 references/audit-framework-and-hunting-paths.md
...
- **工具+多Agent 流水线（2026-08-10 用户要求，治"人工一行行读太累"）**：
  ...
  - **agent 报告读取技巧（2026-08-14 实测）**：delegate 结果 summary 常被截断（[SUMMARY TRUNCATED]），完整报告在 `C:\Users\user\AppData\Local\hermes\cache\delegation\subagent-summary-<N>-<timestamp>.txt`（read_file 分页补读中间段）；live transcript（`...\cache\delegation\live\<delegation_id>\task-N.log`）只存截断版 summary，别指望从那里拿全文。**agent 证据要回验**：buddyforms 注册链 agent 结论"角色只来自配置"我方 read_file 复核 wp-insert-user.php:236 确认后才采信
...
3. WP 插件特有攻击面：
   ...
   - **★ WP_CACHE 常量跳过 nonce 校验模式（2026-08-14 ajax-login-and-registration-modal-popup 案例）**：nonce 校验函数开头出现 `if (defined("WP_CACHE")) return true;` = **缓存插件（WP Super Cache/W3 Total Cache 等）在 wp-config.php define WP_CACHE，生产站常态开启 → 该函数下所有端点的 nonce 校验静默失效**。判定法：grep `defined("WP_CACHE")` 或 "return true" 出现在 wp_verify_nonce 附近 = 可疑；先看绕过 nonce 后端点做什么——只用 WP 核心函数（wp_signon/check_password_reset_key）= 仅 CSRF 失效低危（ajax-login 案例：4 个 nopriv 认证端点全走 _verify_nonce，但内部分别用 wp_signon/get_password_reset_key/check_password_reset_key，无直接 High Threat → 不交）；若绕过 nonce 后有业务操作（状态变更/文件操作/提权）= 硬洞
