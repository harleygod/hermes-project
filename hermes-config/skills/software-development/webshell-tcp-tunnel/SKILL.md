...
## fscan 挂 SOCKS5 扫大段会失效 + netstat 驱动（实测 2026-08）

**fscan 全段扫描在隧道里基本废了**：fscan 存活探测先 ICMP（SOCKS5 走不了 ICMP，响应率 0%）→ 回退 TCP 补充探测，但挂 SOCKS5 时该回退不可靠 → 输出 `存活主机数: 0 / 已扫描 0 个目标`，几秒就"扫完"（**假结果**）。只有目标段里有大量 80 端口时才会被误判为"有存活"（本次 28 段扫到 20 个 80 所以成功，29/30/31 段全是 SQL/MySQL 非 80 服务，全 0）。别把"0 目标"当成"段里没机器"。

**正确姿势：netstat 驱动，别用主动全段扫描**：
1. webshell 上跑 `netstat -ano`，解析**本机（webshell 所在机）的 ESTABLISHED 连接**——它的应用池已经连了所有它需要的服务，远端 `IP:端口` 就是最准的内网服务清单
2. **零隧道压力**（netstat 是本地一条命令，不占 HTTP 隧道），且**不会漏改端口的服务**（主动扫描只扫固定端口列表，改端口的服务扫不到；netstat 显示真实端口）
3. **改端口判断**：标准化托管农场（provisioning 自动化批量部署）几乎不改默认端口（改端口增加管理复杂度，内网本就有防火墙没动力改）。看 netstat 端口是否整齐（全 1433/3306/80）即可定案——本次 20 台 SQL 全 1433、MySQL 全 3306，确认无改端口
4. 对 netstat 新发现的 IP:端口再**慢速单线程针对性探测**（requests 逐个 GET + sleep 间隔），绝不并发压隧道
